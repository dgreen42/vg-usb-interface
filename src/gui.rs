...
pub fn create_window() -> (App, Receiver<Message>, (Vec<Choice>,  IntInput), (Vec<Button>, Input, Output), Table) {
    ...
    let mut base_grid = Grid::default().with_size(1000, 500)
        .center_of_parent();
    base_grid.set_layout_ext(2, 1, 10, 10);

    let mut sub_grid_upper = Grid::default().with_size(800, 200)
        .center_of_parent();
    ...
    let mut sub_grid_bottom = Grid::default().with_size(800, 300)
        .center_of_parent();
    ...
    let base_grid_upper_grid_result = base_grid.set_widget(&mut sub_grid_upper, 0, 0);
    log_error(base_grid_upper_grid_result, "base_grid_upper_grid_result");
    //failed
    let base_grid_bottom_grid_result = base_grid.set_widget(&mut sub_grid_bottom, 1, 0);
...
fn create_data_table() -> (Grid, Table) {
    let mut data_table_grid = Grid::default() .with_size(350, 300);
